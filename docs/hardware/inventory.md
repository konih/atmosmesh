...
| VEML7700 and SHT41 breakouts — pull-ups read off the boards | 1 each inspected | I²C | Operator reading, 2026-09-04, not photo-verified: the VEML7700 breakout carries a part marked `103` (10 kΩ if it is a resistor: black body, white digits; a capacitor would be unmarked), the SHT41 breakout has an **empty** footprint silkscreened `103`, i.e. pull-up pads left unpopulated. Consequence for the Spot: the SHT41 adds no pull-up, the VEML7700 adds 10 kΩ per line if `103` is a two-element resistor array or if there are two of them. **Confirm with the meter:** unpowered, VEML7700 `SDA` to `VIN` and `SCL` to `VIN` should each read about 10 kΩ; SHT41 the same pairs read open. Bus totals in [atmosmesh-spot/wiring.md](../../hardware/kicad/atmosmesh-spot/wiring.md) |
