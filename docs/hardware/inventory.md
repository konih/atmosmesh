...
| ESP32-C3 SuperMini with 0.42" OLED — **photo-verified pin map** | 1 inspected | USB-C, 2 × 8 pins | Silkscreen, USB on the left, OLED up. **Row A (PWR-LED side, `IO8` LED beside the first pin):** `5V GD 3V RX TX 2 1 0` from the USB end. **Row B:** `10 9 8 7 6 5 4 3` from the USB end. So `RX` = GPIO20 and `TX` = GPIO21 sit next to `3V`, and GPIO3/GPIO4 are on the far side — this is **not** the plain SuperMini's map. `BOOT` and `RST` buttons and a small red part with a white dot (presumed antenna) at the end opposite the USB; `PWR` LED at the USB end. **Pin rows 7 pitches (17.78 mm) apart, dry-fit confirmed:** the photo shows the module pushed into the 14 × 20 perfboard with three free columns on each side. OLED I²C pins are not printed; a bus scan settles them. The purple perfboard in the same photo is the operator's 14 × 20 (silkscreen "4cm×6cm 2.54MM"), with a strip of elongated edge pads along each short edge |
